...
commit() {
  ...
  git commit -m "Updated Gerbers: $dateAndMonth (Build $TRAVIS_BUILD_NUMBER)" -m "[skip ci]" > /dev/null 2>&1
}
...
upload_files() {
  ...
  git remote add origin https://thomasb9511:${GH_TOKEN}@github.com/w4xzr/Boards.git > /dev/null 2>&1
